<x-app-layout>
    ...
    <div class="py-12" x-data="{ deleteUrl: '' }">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    @forelse ($pedidos as $pedido)
                        <div class="flex flex-col justify-between border border-gray-200 p-5 rounded-lg hover:shadow-lg transition bg-gray-50">
                            <div>
                                <h3 class="font-bold text-xl text-gray-900 mb-2">{{ $pedido->nome }}</h3>
                                <p class="text-sm text-indigo-600 font-medium flex items-center">
                                    <span class="mr-1">📞</span> {{ $pedido->telefone }}
                                </p>
                                @if($pedido->endereco)
                                    <p class="text-sm text-gray-600 mt-1">
                                        <span class="font-semibold text-gray-800">Endereço:</span> {{ $pedido->endereco }}
                                    </p>
                                @endif
                            </div>
                            ...
                            <div class="flex items-center justify-end mt-6 pt-4 border-t border-gray-200 space-x-4">
                                ...
                                <button 
                                    type="button"
                                    x-on:click="deleteUrl = '{{ route('pedidos.destroy', $pedido->id) }}'; $dispatch('open-modal', 'confirm-pedido-deletion')"
        ...
        <x-modal name="confirm-pedido-deletion" :show="false" focusable>
            <form method="post" :action="deleteUrl" class="p-6">
                @csrf
                @method('delete')

                <h2 class="text-lg font-medium text-gray-900">
                    {{ __('Tem certeza que deseja excluir este pedido?') }}
                </h2>

                <p class="mt-1 text-sm text-gray-600">
                    {{ __('Esta ação não pode ser desfeita. Todos os dados associados a este pedido serão removidos permanentemente.') }}
                </p>

                <div class="mt-6 flex justify-end">
                    <x-secondary-button x-on:click="$dispatch('close-modal', 'confirm-pedido-deletion')">
                        {{ __('Cancelar') }}
                    </x-secondary-button>
